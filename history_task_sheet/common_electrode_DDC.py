...
from my_utils.model_DDC import DDC, mmd_rbf
...
for cross_id in range(NFold):
    ...
    my_net = DDC(transfer_matrix_target)
    # setup optimizer
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            s_class_output, s_feature, _, _ = my_net(input_data=s_eeg, domain=0)
            # 源域的分类误差
            ...
            t_class_output, t_feature, _, _ = my_net(input_data=t_eeg, domain=1)
            ...
            if len(s_label) == len(t_label):
                global_loss = mmd_rbf(s_feature, t_feature, kernel_mul=5.0, kernel_num=10, fix_sigma=None)
            ...
            if config.getint('debug', 'isdebug'):
                sys.stdout.write(
                    '\r epoch: %d, [iter: %d / all %d], err_s_class: %f, err_t_class:%f' \
