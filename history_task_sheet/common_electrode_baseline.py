...
for cross_id in range(NFold):
    ...
    if args.prior_information == '1':
        transfer_matrix_source = torch.eye(config.getint('settings', 'source_num_channel')).to(
            torch.float32).cuda()
        transfer_matrix_target = torch.tensor(
            np.load(os.path.join('config', config.get('settings', 'file_name_transfer_matrix')))).to(
            torch.float32).cuda()
    ...
    for epoch in range(n_epoch):
        ...
        print('\n')
        acc_source = test(test_list=source_eval_list, torch_model=my_net, domain=0, align_transform=None, num_channel=config.getint('settings', 'source_num_channel'))
        print('Cross: %d, Epoch: %d. Accuracy of the %s validation set: %f' % (cross_id, epoch, "Source", acc_source))

        acc_target = test(test_list=target_eval_list, torch_model=my_net, domain=1, align_transform=None, num_channel=config.getint('settings', 'target_num_channel'))
