...
from my_utils.model_EEGNet_withEEGInfinityAPI import EEG_Infinity
from my_utils.test_MengData_transform import test
...
from my_utils.EA_RA2 import RiemannMeanCovariance
...
for cross_id in range(NFold):
    ...
    print("A2")
    source_test_EA = RiemannMeanCovariance(np.load(os.path.join(source_eeg_root, source_test_list[0])), is_cuda=True)
    print("A3")
    source_eval_EA = RiemannMeanCovariance(np.load(os.path.join(source_eeg_root, source_eval_list[0])), is_cuda=True)
    ...
    print("B2")
    target_test_EA = RiemannMeanCovariance(np.load(os.path.join(target_eeg_root, target_test_list[0])), is_cuda=True)
    print("B3")
    target_eval_EA = RiemannMeanCovariance(np.load(os.path.join(target_eeg_root, target_eval_list[0])), is_cuda=True)
    ...
    transfer_matrix_source = torch.eye(config.getint('settings', 'source_num_channel'))

    transfer_matrix_target = torch.tensor(np.load(os.path.join('config', config.get('settings', 'file_name_transfer_matrix')))).to(
            torch.float32).cuda()
    ...
    for epoch in range(n_epoch):
        ...
        print('\n')
        acc_source = test(test_list=source_eval_list, torch_model=my_net, domain=0, align_transform=source_eval_EA, num_channel=config.getint('settings', 'source_num_channel'))
        print('Cross: %d, Epoch: %d. Accuracy of the %s validation set: %f' % (cross_id, epoch, "Source", acc_source))

        acc_target = test(test_list=target_eval_list, torch_model=my_net, domain=1, align_transform=target_eval_EA, num_channel=config.getint('settings', 'target_num_channel'))
    ...
    test_acc_source = test(test_list=source_test_list,
                                               ...
                                               align_transform=source_test_EA,
                                               num_channel=config.getint('settings', 'source_num_channel'))
    ...
    test_acc_target = test(test_list=target_test_list,
                                               ...
                                               align_transform=target_test_EA,
                                               num_channel=config.getint('settings', 'target_num_channel'))
