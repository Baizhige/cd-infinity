...
from my_utils.my_tool import CustomLRScheduler, generate_normalized_tensor_eye, cov_loss
...
parser.add_argument('--config', default='config_MengExp3ToMengExp12.ini', help='Path to the config.ini file')
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            if len(s_label) == len(t_label):
                ...
                cov_distance = cov_loss(s_spatial_output, t_spatial_output) * weight_s(p, a_s, b_s) / 1000
            ...
            if config.getint('debug', 'isdebug'):
                sys.stdout.write(
                    '\r epoch: %d, [iter: %d / all %d], err_s_class: %f, err_t_class:%f, err_s_domain: %f, err_t_domain: %f, l1_distance: %f, cov_distance: %f' \
                    % (epoch, i + 1, len_dataloader, err_s_label.data.cpu().numpy(), err_t_label.data.cpu().numpy(),
                       err_s_domain.data.cpu().numpy(), err_t_domain.data.cpu().item(), l1_distance.data.cpu().item(), cov_distance.data.cpu().item()))
                sys.stdout.flush()
