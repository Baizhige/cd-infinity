...
from my_utils.model_DeepCoral import DeepCoral, euclidean_dist
...
for cross_id in range(NFold):
    ...
    if args.prior_information == '1':
        ...
        transfer_matrix_source = torch.eye(config.getint('settings', 'source_num_channel')).to(
                torch.float64).cuda()
        ...
        transfer_matrix_source = generate_normalized_tensor_eye(config.getint('settings', 'source_num_channel'),
                                                                       config.getint('settings', 'source_num_channel')).cuda()
        transfer_matrix_target = generate_normalized_tensor_eye(config.getint('settings', 'source_num_channel'),
                                                                       config.getint('settings', 'target_num_channel')).cuda()

    my_net = DeepCoral(transfer_matrix_target)
    # setup optimizer
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            s_class_output, s_feature_1, s_feature_2, s_feature_3 = my_net(input_data=s_eeg, domain=0)
            # 源域的分类误差
            ...
            t_class_output, t_feature_1, t_feature_2, t_feature_3 = my_net(input_data=t_eeg, domain=1)
            ...
            if len(s_label) == len(t_label):
                loss_1 = euclidean_dist(s_feature_1, t_feature_1)/30000
                loss_2 = euclidean_dist(s_feature_2, t_feature_2)/30000
                loss_3 = euclidean_dist(s_feature_3, t_feature_3)/20000
                final_loss = err_s_label + (loss_1 + loss_2 + loss_3)
            else:
            ...
            if config.getint('debug', 'isdebug'):
                sys.stdout.write(
                    '\r epoch: %d, [iter: %d / all %d], err_s_class: %f, err_t_class:%f, loss_1:%f, loss_2:%f, loss_3:%f' \
                    % (epoch, i + 1, len_dataloader, err_s_label.data.cpu().numpy(), err_t_label.data.cpu().numpy(), loss_1.data.cpu().numpy(), loss_2.data.cpu().numpy(), loss_3.data.cpu().numpy()))
                sys.stdout.flush()
