...
class EEG_Infinity(nn.Module):
    def __init__(self, transfer_matrix_source, transfer_matrix_target, num_channels = 64, FIR_order=17, FIR_n=1, backbone_type='InceptionEEG',right_idx=None, left_idx=None):
        ...
        # define prototype mapper
        self.prototype_mapper = PrototypeMapper(num_prototypes=2, feature_dim=self.feature_map_size)


    def forward(self, input_data, domain, alpha, label=None):
        ...
        if label is not None:
            prototype_loss = self.prototype_mapper.get_prototype_loss(_feature_, label)
        else:
            prototype_loss = None
        return class_output, domain_output, prototype_loss, None


class PrototypeMapper(nn.Module):
    def __init__(self, num_prototypes, feature_dim, gamma=1000):
        """
        初始化 PrototypeMapper 模块。

        参数:
        - num_prototypes (int): 原型的数量，通常对应类别的数量。
        - feature_dim (int): 特征的维度。
        - gamma (float): 控制相似度计算的温度参数，默认为1.0。
        """
        ...
        # 初始化原型 P，形状为 (num_prototypes, feature_dim)
        # 使用正态分布随机初始化，并将其设为可训练参数
        self.prototypes = nn.Parameter(torch.randn(num_prototypes, feature_dim))

        # 定义原型映射器 G_p，这里使用一个简单的线性层
        # 也可以根据需要更换为更复杂的网络结构
    ...
    def forward(self, x):
        ...
        参数:
        - x (Tensor): 输入特征，形状为 (batch_size, feature_dim)

        返回:
        - mapped_x (Tensor): 映射后的特征，形状为 (batch_size, feature_dim)
        """
        mapped_x = self.mapper(x)  # 形状保持为 (batch_size, feature_dim)
        return mapped_x
    ...
    def get_prototype_loss(self, x, y):
        ...
        参数:
        - x (Tensor): 输入特征，形状为 (batch_size, feature_dim)
        - y (Tensor): 标签，形状为 (batch_size, 1)，包含0或1的类别标签
        ...
        # 计算每个样本与所有原型的欧氏距离的平方，形状为 (batch_size, num_prototypes)
        # 使用广播机制进行计算
        # mapped_x.unsqueeze(1): (batch_size, 1, feature_dim)
        # self.prototypes.unsqueeze(0): (1, num_prototypes, feature_dim)
        distances = torch.norm(mapped_x.unsqueeze(1) - self.prototypes.unsqueeze(0),
                               dim=2) ** 2  # (batch_size, num_prototypes)

        # 计算相似度 o(x_i, p_k) = exp(-||G_p(x_i) - p_k||^2 / gamma)
        o = torch.exp(-distances / self.gamma)  # (batch_size, num_prototypes)

        # 将 y 转换为 one-hot 编码，形状为 (batch_size, num_prototypes)
        # 假设 y 的类别标签从 0 到 num_prototypes - 1
        T = torch.zeros(batch_size, self.num_prototypes, device=x.device)
        T.scatter_(1, y, 1)

        # 计算损失
        # 使用公式：-T * log(o) - (1 - T) * log(1 - o)
        # 为避免 log(0) 的情况，加入一个小的常数 1e-8
        loss = -T * torch.log(o + 1e-8) - (1 - T) * torch.log(1 - o + 1e-8)  # (batch_size, num_prototypes)
...
class FIR_convolution(nn.Module):
    def __init__(self, FIR_n, FIR_order):
        ...
        # Create a 2D convolutional layer to implement a FIR filter
...
if __name__ == "__main__":
    a = scio.loadmat(os.path.join('dataset_config_files', 'transfer_62To64.mat'))['transform_matrix']
