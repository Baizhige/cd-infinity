...
from .model_utils.model_standard_deep4_functions import (
    identity, safe_log, square, transpose_time_to_spat
)
from torch.nn import init


class DANet_1DConv(nn.Module):
    def __init__(self, kernel_size=17, n_chan=None):
        """
        DANet_1DConv 网络，仅对时间维度进行一维卷积，保持输入和输出形状不变。

        参数:
        - kernel_size (int): 卷积核的宽度。默认值为3。
        """
        super(DANet_1DConv, self).__init__()
        self.kernel_size = kernel_size

        # 定义卷积层，卷积核大小为 (1, kernel_size)，仅在时间维度上卷积
        # 设置 padding=(0, kernel_size//2) 以保持时间维度不变
        self.conv1 = nn.Conv2d(
            in_channels=1,
            out_channels=1,
            kernel_size=(1, kernel_size),
            padding=(0, kernel_size // 2),
            bias=False  # 通常在卷积层后使用归一化层时不使用偏置
        )
    ...
    def forward(self, x):
        ...
        参数:
        - x (torch.Tensor): 输入张量，形状为 (batch_size, 1, num_channels, timeseries_len)

        返回:
        - out (torch.Tensor): 输出张量，形状与输入相同
        """
        # 保存输入用于残差连接
        residual = x

        # 卷积操作
        out = self.conv1(x)  # (batch_size, 1, num_channels, timeseries_len)
        out = self.bn1(out)  # 批归一化
        # out = self.relu(out)  # 激活函数

        # 添加残差连接（可选）
        out = residual + 0.01 * out
...
class DANet(nn.Module):
    def __init__(self, n_chan=8, fc_in=8, latsize=64):
        ...
        self.Bn1 = nn.BatchNorm2d(1)
        # self.In1 = nn.InstanceNorm2d(1, affine=True)  # 可选的实例归一化层
    ...
    def forward(self, x):
        # Encoder
        x = self.conv1(x)  # 卷积层
        x = torch.permute(x, (0, 2, 1, 3))  # 维度重排
        x = self.Bn1(x)  # 批归一化
        latent = torch.permute(x, (0, 1, 3, 2))  # 再次维度重排
        latent = self.FC(latent)  # 全连接层
        out = torch.permute(latent, (0, 1, 3, 2))  # 最后一次维度重排
        return out

class EEG_Infinity(nn.Module):
    '''
    No EEG Infinity core and only with its API. This class is only for comparison study.
    '''

    def __init__(self, transfer_matrix_source, transfer_matrix_target, num_channels = 64, FIR_order=17, FIR_n=1,
                 backbone_type='InceptionEEG', right_idx=None, left_idx=None):
    ...
    def forward(self, input_data, domain, alpha):
        ...
        __feature__ = self.feature(filter_output).view(-1, self.feature_map_size)
        class_output = self.class_classifier(__feature__)

        return class_output, filter_output, None, None
...
class Alignment_head(nn.Module):
    def __init__(self, transfer_matrix, is_source, n_chan):
        super(Alignment_head, self).__init__()
        self.channel_transfer_matrix_fixed = transfer_matrix.cuda()
        self.is_source = is_source
        if self.is_source:
            self.danet = DANet_1DConv(n_chan=n_chan)
    def forward(self, input_data):
        _input_data_ = input_data.to(torch.float32)
        _output_ = torch.matmul(self.channel_transfer_matrix_fixed, _input_data_)
        if self.is_source: # transfer source domain to target domain
            _output_ = self.danet(_output_)
        return _output_, None
...
class ShallowNetFeatureExtractor(nn.Module):
    def __init__(self,
                 ...
                 drop_prob=0.5):  # keep parameters
        super(ShallowNetFeatureExtractor, self).__init__()
        # just change API, do not change parameters
