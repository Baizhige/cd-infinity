...
class EEGDataSet(data.Dataset):
    def __init__(self, data_root, data_list,
        ...
        # self.num_channel = num_channel # for sex paradigm, control this
        self.transforms = transforms
        self.channel_select = channel_select
        if max_sample is not None:  # If control sample size
            self.eeg_data = np.load(os.path.join(data_root, data_list[0]))[: max_sample]
            self.eeg_labels = np.load(os.path.join(data_root, data_list[1]))[: max_sample]
        elif max_sample is None:
    ...
    def __getitem__(self, item):
        ...
        if self.channel_select is not None:
            eegs = eegs[self.channel_select, :]
        if self.isChannelNorm:
            # 对每个 channel 进行标准化：均值为 0，方差为 1
            mean = np.mean(eegs, axis=1, keepdims=True)  # shape: (channels, 1)
            std = np.std(eegs, axis=1, keepdims=True)  # shape: (channels, 1)
            std[std == 0] = 1  # 防止除以0
            eegs = (eegs - mean) / std
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_config_files', default='config_PhysioNetMIToMengExp3.ini', help='Path to the dataset_config_files.ini file')
    ...
    batch2_sample_target, subjects, labels = next(target_train_iter)
