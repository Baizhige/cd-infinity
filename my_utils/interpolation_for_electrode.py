...
shape = "(62_64)"
T_replace  = np.load(os.path.join("..","dataset_config_files",f"transformation_matrix_{orinal_dataset}_{transform_dataset}_{shape}_null.npy"))
print(np.sum(T_replace, axis=1))
Adj = np.load(os.path.join("..","dataset_config_files",f"{transform_dataset}_Geo_Adj.npy"))
S = np.diag(find_empty_rows(T_replace))
T_int = get_interpolation_matrix(Adj=Adj,S=S)
T_final = T_int @ T_replace
np.save(os.path.join("..","dataset_config_files",f"transformation_matrix_{orinal_dataset}_{transform_dataset}_{shape}_new.npy"), T_final)
