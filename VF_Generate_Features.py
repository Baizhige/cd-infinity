...
def process_method(method, cross_id, task):
    ...
    s_FC_features = torch.zeros((len_dataloader * config.getint('settings', 'batch_size'), model.feature_map_size))
    t_FC_features = torch.zeros((len_dataloader * config.getint('settings', 'batch_size'), model.feature_map_size))
    ...
    for i in range(len_dataloader):
        ...
        s_eeg, _ = model.alignment_head_source(s_eeg)
        if hasattr(model, 'channel_norm'):
            s_eeg = model.channel_norm(s_eeg)
        s_FC_feature = model.feature.feature(s_eeg).view(-1, model.feature_map_size)
        s_FC_features[i * 32: (i + 1) * 32, :] = s_FC_feature.squeeze()  # 使用squeeze确保维度匹配

        data_target = next(data_target_iter)
        t_eeg, t_subject, t_label = data_target
        t_eeg, _ = model.alignment_head_target(t_eeg)
        if hasattr(model, 'channel_norm'):
            t_eeg = model.channel_norm(t_eeg)
        t_FC_feature = model.feature.feature(t_eeg).view(-1, model.feature_map_size)
        t_FC_features[i * 32: (i + 1) * 32, :] = t_FC_feature.squeeze()  # 使用squeeze确保维度匹配

    print(s_FC_features.size())
    print(t_FC_features.size())

    torch.save(s_FC_features, os.path.join("record", f"{method}_s_FC_features_stacked.pth"))
    torch.save(t_FC_features, os.path.join("record", f"{method}_t_FC_features_stacked.pth"))
