...
from my_utils.model_Infinity002API_any_backboneDANet import EEG_Infinity
from my_utils.test_script import test
...
parser.add_argument('--dataset_config_files', default='config_PhysioNetMIToMengExp12.ini', help='Path to the dataset_config_files.ini file')
...
config.read(os.path.join("../ini_config", args.config))
...
for cross_id in range(NFold):
    ...
    # 获取 source_channel_select
    if config.has_option('settings', 'file_source_order'):
        source_channel_path = os.path.join("..", "dataset_config_files", config.get('settings', 'file_source_order'))
        source_channel_select = np.load(source_channel_path)
    else:
        source_channel_select = None

    # 获取 target_channel_select
    if config.has_option('settings', 'file_target_order'):
        target_channel_path = os.path.join("..", "dataset_config_files", config.get('settings', 'file_target_order'))
        target_channel_select = np.load(target_channel_path)
    else:
        target_channel_select = None
    source_train_dataset = EEGDataSet(
        ...
        channel_select=source_channel_select,
    )
    ...
    target_train_dataset = EEGDataSet(
        ...
        channel_select=target_channel_select,
    )
    ...
    _right_idx_ = torch.tensor(np.load(os.path.join("../dataset_config_files", config.get('settings', 'right_idx'))) - 1).cuda()
    _left_idx_ = torch.tensor(np.load(os.path.join("../dataset_config_files", config.get('settings', 'left_idx'))) - 1).cuda()

    CAR_matrix_target = torch.eye(config.getint('settings', 'target_num_channel'))

    transfer_matrix_target = torch.matmul(
        torch.tensor(np.load(os.path.join('../dataset_config_files', config.get('settings', 'file_name_transfer_matrix')))).to(
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            s_class_output, s_filter_out, _, _ = my_net(input_data=s_eeg, domain=0, alpha=alpha)
            # cls of source data
            ...
            t_class_output, t_filter_out, _, _ = my_net(input_data=t_eeg, domain=1, alpha=alpha)

            err_s_label.backward(retain_graph=True)
            my_net.alignment_head_source.danet.zero_grad() # 不用来分类

            # adujust loss for source and target
            if len(s_label) == len(t_label):
                transfer_loss = (1 - alpha) * torch.nn.functional.mse_loss(s_filter_out, t_filter_out)
            ...
            if 1 or config.getint('debug', 'isdebug'):
                sys.stdout.write(
                    '\r epoch: %d, [iter: %d / all %d], err_s_class: %f, transfer_loss: %f, alpha: %f' \
                    % (epoch, i + 1, len_dataloader, err_s_label.data.cpu().numpy(), transfer_loss.data.cpu().numpy(), (1 - alpha)))
                sys.stdout.flush()
        ...
        print('\n')
        acc_source = test(test_list=source_eval_list, torch_model=my_net, domain=0,
                          num_channel=config.getint('settings', 'source_num_channel'),
                          channel_select=source_channel_select
                          )
        print('Cross: %d, Epoch: %d. Accuracy of the %s validation set: %f' % (cross_id, epoch, "Source", acc_source))

        acc_target = test(test_list=target_eval_list, torch_model=my_net, domain=1,
                          num_channel=config.getint('settings', 'target_num_channel'),
                          channel_select=target_channel_select)
...
append_results_to_csv(cache_prefix, record_val_metric, record_test_metric, file_path=os.path.join("../record", "comparison_study_DANet.csv"))
