...
from my_utils.model_EEG_Infinity002API_any_backboneDANNPrototype import EEG_Infinity
...
for cross_id in range(NFold):
    ...
    source_channel_select = np.load(os.path.join("..", "dataset_config_files", config.get('settings', 'file_source_order')))
    target_channel_select = np.load(os.path.join("..", "dataset_config_files", config.get('settings', 'file_target_order')))
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            s_class_output, s_domain_output, s_prototype_loss, _ = my_net(input_data=s_eeg, domain=0, alpha=alpha, label=s_label)
            ...
            err_DANN = err_s_label + err_s_domain + err_t_domain + s_prototype_loss
            err_DANN.backward()
            ...
            if config.getint('debug', 'isdebug'):
                sys.stdout.write(
                    '\r epoch: %d, [iter: %d / all %d], err_s_class: %f, err_t_class:%f, err_s_domain: %f, err_t_domain: %f, s_prototype_loss: %f' \
                    % (epoch, i + 1, len_dataloader, err_s_label.data.cpu().numpy(), err_t_label.data.cpu().numpy(),
                       err_s_domain.data.cpu().numpy(), err_t_domain.data.cpu().item(), s_prototype_loss.data.cpu().item()))
                sys.stdout.flush()
...
append_results_to_csv(cache_prefix, record_val_metric, record_test_metric, file_path=os.path.join("../record", "comparison_study_DANNPrototype.csv"))
