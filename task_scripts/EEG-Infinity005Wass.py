...
parser.add_argument('--dataset_config_files', default='config_sleepcapslpdbTohmcsleepstaging.ini', help='Path to the dataset_config_files.ini file')
...
for cross_id in range(NFold):
    ...
    # load model
    if config.getint('settings', 'source_num_channel') != 1:
    ...
    elif config.getint('settings', 'source_num_channel') == 1:
        CAR_matrix_source = torch.eye(config.getint('settings', 'source_num_channel'))
    else:
        raise ValueError("Something Wrong about source number of channel")
    ...
    if config.getint('settings', 'target_num_channel') != 1:
    ...
    elif config.getint('settings', 'target_num_channel') == 1:
        CAR_matrix_target = torch.eye(config.getint('settings', 'target_num_channel'))
    else:
        raise ValueError("Something Wrong about target number of channel")
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            with torch.no_grad():
                ...
                writer.add_scalar('l1_distance', l1_distance, epoch * len_dataloader + i)
                writer.add_scalar('cov_distance', cov_distance, epoch * len_dataloader + i)
        print('\n')
...
append_results_to_csv(cache_prefix, record_val_metric, record_test_metric, file_path=os.path.join("../record", "extra_sleepstudy_Infinity005Wass.csv"))
